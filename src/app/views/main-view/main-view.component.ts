...
export class MainViewComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	listenForEvents() {
		this.subscriptions.push(this.keys.keypress("keyup.control.s").subscribe(() => {
			this.pfService.saveProject();
		}));
		this.subscriptions.push(this.keys.keypress("keyup.control.z").subscribe(() => {
			this.pfService.undo();
		}));
		this.subscriptions.push(this.keys.keypress("keyup.control.y").subscribe(() => {
			this.pfService.redo();
		}));

		//ctr+0 to 9 to select a track
		for(let i = 0; i < 10; i++) {
			this.subscriptions.push(this.keys.keypress(`keyup.control.${i}`).subscribe(() => {
				this.trackService.selectTrackByIndex(i);
			}));
		}

		//up/down to move the between tracks
		this.subscriptions.push(this.keys.keypress("keyup.arrowup").subscribe(() => {
			this.trackService.selectPreviousTrack();
		}));
		this.subscriptions.push(this.keys.keypress("keyup.arrowdown").subscribe(() => {
